  ...
  const incMeta =
    inc.status === "pass"
      ...
      : inc.status === "tight"
        ...
        : ["FAIL", "#a4261f", "#f9e2df"];
  const rateMeta = rate.pass ? ["PASS", "#176b3f", "#e3f1e6"] : ["FAIL", "#a4261f", "#f9e2df"];
  ...
  return (
    ...
    >
      <ul className="grid gap-2.5">
        {tile(incMeta, <>If income dropped 20%, free cash flow would be <b>{formatINR(inc.newFcf)}/month</b> — {inc.status === "pass" ? "your EMI still fits." : inc.status === "tight" ? "your EMI barely fits. Keep a bigger buffer." : "your EMI would not fit."}</>)}
        {tile(rateMeta, <>If rates rose 2 points, the same loan would cost <b>{formatINR(rate.newEmi)}/month</b> — {rate.pass ? "still inside your cash flow." : "beyond your cash flow."}</>)}
      </ul>
