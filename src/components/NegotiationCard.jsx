...
// result-aware: styled rows + plain fallback for clipboard
export default function NegotiationCard({ result, text }) {
  ...
  const rows = hasResult ? [
    ["Profile", `${r.profile?.incomeType?.replace("_", " ") ?? r.routing?.lane ?? ""} · ${r.cf ? (Array.isArray(r.profile?.income?.amount) ? `${formatINR(r.cf.blendedIncome)}` : formatINR(r.cf.blendedIncome)) + "/mo" : ""} · Credit: ${r.profile?.creditScoreKnown ? r.profile?.creditScore : "unknown"}`],
